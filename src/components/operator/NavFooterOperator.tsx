...
const NavFooterOperator = () => {
  const location = useLocation();
  const isHome = location.pathname === '/operator';

  if (isHome) return null;

  const navItems = [
    { title: 'Inicio', icon: <HomeIcon className="text-2xl md:text-3xl" />, path: '/operator' },
    { title: 'Gestionar Naves', icon: <DirectionsBoatIcon className="text-2xl md:text-3xl" />, path: '/operator/manage-vessels' },
...
export default NavFooterOperator;
